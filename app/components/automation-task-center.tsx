...
export default function AutomationTaskCenter({ automation, recovery, recoveryLoading, durableProjectId, isRunning, backgroundActive, backgroundBusy, onRefreshRecovery, onRecoverStep, onCancelBackground }: Props) {
  ...
  return <>
    <section className="automation-task-center recovery-center">
      ...
      {!durableProjectId ? <p className="automation-task-empty">{"\u4fdd\u5b58\u4e3a\u4e91\u7aef\u4f5c\u54c1\u540e\uff0c\u6bcf\u4e2a\u9636\u6bb5\u68c0\u67e5\u70b9\u90fd\u4f1a\u51fa\u73b0\u5728\u8fd9\u91cc\u3002"}</p> : latestRun ? <>
        <div className="auto-progress-meta"><span><Route size={14} />{"\u6700\u8fd1\u8fd0\u884c\uff1a"}{phaseLabel(latestRun.phase)}</span><span><FileText size={14} />{"\u7b2c "}{latestRun.currentChapterNumber || 0}{" \u7ae0 \u00b7 \u6574\u7ae0\u6b63\u6587\u6b65\u9aa4"}</span><span><Zap size={14} />{latestRun.usage.totalTokens.toLocaleString("zh-CN")} Token</span></div>
        {latestRun.steps.length ? <div className="automation-task-list">{latestRun.steps.slice(0, 8).map((step) => <article key={step.id} className={`task-${step.status}`}><i>{step.status === "completed" ? <Check size={13} /> : <AlertTriangle size={13} />}</i><div><b>{kindLabel(step.kind)}</b><small>{step.chapterNumber ? `\u7b2c ${step.chapterNumber} \u7ae0${step.kind === "chapter_segment" ? " \u00b7 \u6574\u7ae0\u6b63\u6587" : ""}` : "\u84dd\u56fe\u9636\u6bb5"} \u00b7 {step.status === "completed" ? "\u5df2\u4fdd\u5b58" : step.error || "\u5931\u8d25"}</small></div><button className="secondary-button compact" disabled={isRunning} onClick={() => onRecoverStep(step)}>{"\u4ece\u6b64\u7ee7\u7eed"}</button></article>)}</div> : <p className="automation-task-empty">{"\u5df2\u521b\u5efa\u8fd0\u884c\u8bb0\u5f55\uff0c\u6682\u65e0\u53ef\u6062\u590d\u7684\u9636\u6bb5\u68c0\u67e5\u70b9\u3002"}</p>}
      </> : <p className="automation-task-empty">{recoveryLoading ? "\u6b63\u5728\u8bfb\u53d6\u4e91\u7aef\u68c0\u67e5\u70b9\u2026" : "\u6682\u65e0\u4e91\u7aef\u6062\u590d\u8bb0\u5f55\u3002"}</p>}
    </section>
    <section className="automation-task-center">
      <div className="auto-section-title"><div><span>TASK CENTER</span><h3>{"\u540e\u53f0\u4efb\u52a1\u4e2d\u5fc3"}</h3></div><div className="task-center-actions"><small>{(automation.taskLog || []).length}{" \u6761\u8bb0\u5f55"}</small><button className="secondary-button compact cancel-task-button" disabled={!backgroundActive || backgroundBusy} onClick={onCancelBackground}><CircleStop size={14} />{backgroundBusy ? "\u6b63\u5728\u53d6\u6d88" : backgroundActive ? "\u53d6\u6d88\u540e\u53f0\u4efb\u52a1" : "\u5f53\u524d\u65e0\u540e\u53f0\u4efb\u52a1"}</button></div></div>
