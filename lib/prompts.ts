...
const taskInstructions: Record<string, string> = {
  ...
  "情节推进": "基于当前大纲和未回收线索，提供 3 个差异明显的下一步情节方案。每个方案包含触发事件、人物选择、代价和可回收伏笔。",
  "生成章节": "按照当前章节梗概生成 1800—2500 字章节草稿。保持人物动机和世界规则一致，场景中必须发生可见变化，直接输出正文。",
...
export function buildNovelContext(workspace: WorkspaceData, chapterId?: string) {
  ...
  const nearbyChapters = currentChapter
    ? workspace.chapters
        .filter((chapter) => Math.abs(chapter.number - currentChapter.number) <= 2)
        .sort((a, b) => a.number - b.number)
        .map((chapter) => chapter.number <= currentChapter.number ? compactChapter(chapter) : ({
          number: chapter.number,
          title: chapter.title,
          summary: chapter.summary,
          status: chapter.status,
  ...
  return {
    ...
    nearbyChapters,
    unresolvedIssues: workspace.issues.filter((issue) => !issue.resolved),
    foreshadows: workspace.materials.filter((material) => material.type === "伏笔"),
  };
}
...
export function buildUserPrompt(
  ...
  return `你是长篇小说作者的专业创作搭档。请严格尊重已给出的事实，不擅自改名，不把建议当成既定设定。\n\n【任务】\n${task}\n${taskInstruction}\n\n【作者补充要求】\n${instruction || "无额外要求，请依据已有上下文完成。"}\n\n【小说上下文】\n${JSON.stringify(context, null, 2)}\n\n请使用简体中文输出。`;
}
